...
function collecterExif($image_path, $post_id) {
    ...
    if (!is_dir($upload_dir)) {
        return ['error' => "Le dossier d'upload EXIF n'existe pas : $upload_dir"];
    }

    if (!is_writable($upload_dir)) {
        return ['error' => "Le dossier d'upload EXIF n'est pas accessible en écriture : $upload_dir"];
    }
    ...
    if ($image_type === false) {
        return ['error' => "Impossible de détecter le type de l'image. Assurez-vous que le fichier est valide : $image_path"];
    }

    if (!in_array($image_type, [IMAGETYPE_JPEG, IMAGETYPE_PNG], true)) {
        return ['error' => "Type d'image non supporté. Seuls JPEG et PNG sont autorisés. Type détecté : $image_type"];
    }
    ...
    if (empty($output)) {
        return ['error' => "Le script Python n'a retourné aucune donnée. Vérifiez l'image ou le script Python."];
    }
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        ...
        return ['error' => "Les métadonnées EXIF retournées par Python sont invalides. Erreur JSON : " . json_last_error_msg()];
    }
    ...
    // Retourne les métadonnées collectées
    return $metadata;
}
